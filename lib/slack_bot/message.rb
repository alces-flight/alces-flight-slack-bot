...
module SlackBot
  class Message
    class << self
      def process(data)
        Message.new(YAML.load(data)).process
      rescue
        Result.new({error: $!.message, backtrace: $!.backtrace}, Result::FAILURE)
      end
    end
    ...
    def process
      ...
      when Hash
        case @data['type']
        when 'say'
          Say.new(@data).process
          Result.new
        when 'nagios'
          Nagios.new(@data).process
          Result.new
        else
          raise "Unrecognised message type: #{@data['type']}"
        end
      else
        raise "Invalid message type: #{@data.class}"
      end
    end
  end
end
